...
print(len(train_x))
_, idx = train_obj.squeeze().topk(100)
train_x = train_x[idx]
_, idx = np.unique(train_x, return_index=True)
topsmiles = decode(train_x.numpy())

with open(f'{TARGET}-best_smiles.txt', 'w') as f:
    f.write('\n'.join(list(set(topsmiles))))
...
Draw.MolsToGridImage(ms, molsPerRow=5, subImgSize=(400, 400)).save(f'{TARGET}-best_smiles.pdf')