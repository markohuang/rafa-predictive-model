...
x0 = df['smiles']
y0 = df[args.target]
x0.to_csv(os.path.join(args.target, 'all.txt'), header=None, index=None, sep=' ')
np.save(os.path.join(args.target, 'all_labels'), np.array(y0.values))
# Train
x1 = train['smiles']
y1 = train[args.target]
x1.to_csv(os.path.join(args.target, 'train.txt'), header=None, index=None, sep=' ')
np.save(os.path.join(args.target, 'train_labels'), np.array(y1.values))
# Val
x2 = val['smiles']
y2 = val[args.target]
x2.to_csv(os.path.join(args.target, 'val.txt'), header=None, index=None, sep=' ')
np.save(os.path.join(args.target, 'val_labels'), np.array(y2.values))
# Test
x3 = test['smiles']
y3 = test[args.target]
x3.to_csv(os.path.join(args.target, 'test.txt'), header=None, index=None, sep=' ')
