...
TARGET = args.target
D = 56  # latent_size
X = np.loadtxt(f'./bo/{TARGET}-latent_features.txt')
...
def get_initial_data(n=5):
    ...
    train_x = np.loadtxt(f'./bo/{TARGET}-latent_features.txt')
    train_obj = np.loadtxt(f'./bo/{TARGET}-scores.txt')[:, np.newaxis] # add output dimension
...
with open(f'bo/{TARGET+str(seed)}-best_smiles.txt', 'w') as f:
    f.write('\n'.join(topsmiles))

with open(f'bo/{TARGET+str(seed)}-best_scores.txt', 'w') as f:
...
print(topsmiles)
suppl = Chem.SmilesMolSupplier(f'bo/{TARGET+str(seed)}-best_smiles.txt', titleLine=False)
...
with open(f'bo/{TARGET+str(seed)}-best_smiles.svg', 'w') as f:
    f.write(svg)
