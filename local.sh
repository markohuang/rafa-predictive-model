TARGET=homo
SOURCEDIR=`pwd`
SLURM_JOB_ID=local
export PYTHONPATH=$PYTHONPATH:$SOURCEDIR

# Setup data
# cd $SOURCEDIR/data/rafa
# python setup_data.py --target $TARGET

# Get vocab
# cd $SOURCEDIR/auxiliaries
# python ../jtnn/mol_tree.py < ../data/rafa/all.txt > ../data/rafa/vocab.txt

# Preprocess
# python preprocess.py --target $TARGET --jobs 16

# Train
LATENT_SIZE=56
HIDDEN_SIZE=450
BATCH_SIZE=32
EPOCH=15
MODEL_DIR=model-$TARGET/$SLURM_JOB_ID-lsize$LATENT_SIZE-hsize$HIDDEN_SIZE-bsize$BATCH_SIZE-e$EPOCH/
mkdir -p $MODEL_DIR
python main.py \
--vocab ./data/rafa/vocab.txt \
--save_dir $MODEL_DIR \
--latent_size $LATENT_SIZE \
--hidden_size $HIDDEN_SIZE \
--batch_size $BATCH_SIZE \
--save_iter 500 \
--epoch $EPOCH > ${MODEL_DIR}output.txt

wait
# Test and plot
python test_and_plot.py --model_dir $MODEL_DIR

